...
%     'maxNumIter',30,'gamma',0.1, 'updatepriors', 0, 'updateVis', 0, 'updateTR', 1, ...
%     'normal', N, 'normalLambda', .0, 'color', C, 'colorLambda', .0);

[R,t,XX,S2,a2,pk2,T2] = jrmpc(V, Xin,'maxNumIter',30,'gamma', 0.1);%, 'updateTR', 0);
...
%     scatter3(V{i}(1,:), V{i}(2,:), V{i}(3,:), 8, colors(i,:), 'filled');
end
axis equal;
...
%         h1 = plot_gaussian_ellipsoid(XX3(:,i), eye(3).*S2(i), vw);
        ...
        h1 = plot_gaussian_ellipsoid(real(XX(:,i)), real(S(:,:,i)), vw);
...
for di = 1:length(VV)
    vall = [vall VV{di}];
    call = [call CC{di}];
    sall = [sall repmat(colors(di, :)', 1, size(VV{di}, 2))];
    dall = [dall DD{di}];
end
dataOutRoot = '/home/mhlee/data_from_odroid/merged';
codeName = sprintf('Convert_to_model_with_S_%f', sLambda);
fileName = sprintf('%s_seg', codeName);
fcn_saveUniformSizeModel( (vall+dall)', sall', [], dataOutRoot, fileName);
fileName = sprintf('%s_seg_ori', codeName);
fcn_saveUniformSizeModel( (vall)', sall', [], dataOutRoot, fileName);
fileName = sprintf('%s_color', codeName);
fcn_saveUniformSizeModel( (vall+dall)', call', [], dataOutRoot, fileName);
fileName = sprintf('%s_color_ori', codeName);
fcn_saveUniformSizeModel( (vall)', call', [], dataOutRoot, fileName);
