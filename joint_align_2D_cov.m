...
% XX = [];
% for i = 1:cSize
%     XX = [XX ; bsxfun(@plus, X, [0 0 i])];
%     XX = [XX ; bsxfun(@plus, Y, [i 0 0])];
% end
% 
% 
% 
...
%     'maxNumIter',2,'gamma',0.1, 'updatepriors', 0, 'updateVis', updateVis, 'updateTR', 0);

[R,t,X2,S,a,pk,T,TAssigned, TXQ, vis, ~, ~] = jrmpc_soft_with_cov_diag(V,Xin, ...
    'maxNumIter',maxNumIter,'gamma',0.1, 'updatepriors', 0, 'updateVis', updateVis, 'updateTR', 1);
% [R,t,X2,S,a,pk,T,TAssigned, TXQ, vis, ~, ~] = jrmpc_soft_with_normal(V,Xin, ...
%     'maxNumIter',maxNumIter,'gamma',0.1, 'updatepriors', 0, 'updateVis', updateVis, 'updateTR', 1);
...
figure; 
scatter3(TV{1}(1,:), TV{1}(2,:), TV{1}(3,:), 8, 'r', 'filled');
hold on;
scatter3(TV{2}(1,:), TV{2}(2,:), TV{2}(3,:), 8, 'b', 'filled');
for i=1:size(X2, 2)
    h1 = plot_gaussian_ellipsoid(real(X2(:,i)), real(S(:,:,i)), 1);
