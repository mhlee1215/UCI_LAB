...
XX = [];
for i = 1:cSize
    XX = [XX ; bsxfun(@plus, X, [0 0 i*5])];
%     XX = [XX ; bsxfun(@plus, Y, [i*0.3 0 0])];
end
...
[R,t,X2,S,a,pk,T,TAssigned, TXQ, vis, ~, ~] = jrmpc_soft_with_normal(V,Xin, ...
    'maxNumIter',maxNumIter,'gamma',0.1, 'updatepriors', 1, 'updateVis', updateVis, 'updateTR', 0);
