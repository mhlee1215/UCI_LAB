...
%     V2{end+1} = fv{data_idx}.vertices(rand_idx(1:sampleNum), :)';
    ...
    fv{data_idx}.Faces = fv{data_idx}.faces;
    fv{data_idx}.Vertices = fv{data_idx}.vertices;
...
%     V2{end+1} = randSubSample(fv{data_idx}.vertices, sampleNum);
    [sData, sIndex, sColor] = uniformSubSample(fv{data_idx}.vertices, 70);
    I2{end+1} = sIndex;
...
vis = genVisFromPeriod(st5, M, K, epsilonST);
vis = cell2mat(vis');
...
paramsPlotGMMK.maxIdx = TAssigned(:,end);
...
[R2,t2,X2,S2,a2,pk2,T2,TAssigned2, TXQ2] = jrmpc_soft(V,Xin,'maxNumIter',maxNumIter,'gamma',0.1, 'updatepriors', 0, 'updateVis', 0);
params.type = 1;
...
params.pause = .0;
h = figure;
[TV] = drawTransformation(V, T2, params);
axis equal;
