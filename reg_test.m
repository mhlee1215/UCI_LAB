...
%     fv{data_idx} = stlread(sprintf('%s/%s_%.2f.%s', crop_path, data_name, dist_set{data_idx}, data_ext));
    ...
    crop_path = sprintf('%s', data_path);
    fv{data_idx} = stlread(sprintf('%s/%s.%s', crop_path, data_name, data_ext));
...
% [R,t,X,S,a,pk,T] = jrmpc(V,Xin,'maxNumIter',maxNumIter,'gamma',0.1, 'updatepriors', 1);
 [R,t,X,S,a,pk,T] = jrmpc2(V,Xin,'maxNumIter',maxNumIter,'gamma',0.1, 'updatepriors', 1);
...
figure;scatter3(X(1,:),X(2,:),X(3,:),40,ddd2,'filled'); colorbar; title('mean dist to GMMs');
figure;scatter3(X(1,:),X(2,:),X(3,:),40,pk,'filled'); colorbar; title('pk');
figure;scatter3(X(1,:),X(2,:),X(3,:),40,S,'filled'); colorbar; title('Variance');