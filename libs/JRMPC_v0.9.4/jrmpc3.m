function [R,t,X,Q,a,pk,T] = jrmpc3(V,X,varargin)
...
%     a = cellfun(@(a) bsxfun(@times,pk.*(Q'.^1.5),exp(bsxfun(@times,-.5*Q',a))),a,'uniformoutput',false);
    ...
    a3 = cellfun(@(a1, a2, a3) (a1+a2+a3) ./ 3 , a31, a32, a33,'uniformoutput',false);

    % normalize
    a = cellfun(@(a) bsxfun(@rdivide,a,sum(a,2)+beta),a3,'uniformoutput',false);    
