function [R,t,X,Q,a,pk,T] = jrmpc2_gpu(V,X,varargin)
...
%     a = cellfun(@(a) gpuArray(a), a, 'uniformoutput',false);
    ...
    den = sum(cell2mat(cg2cm(lambda)'),2)'; % den is used for S's update as well
    den2 = sum(cell2mat(cg2cm(lambda2)'),2)'; % den is used for S's update as well
