...
[R,t,X,S,a,pk,T,Assigned] = jrmpc2(V,Xin,'maxNumIter',maxNumIter,'gamma',0.1, 'updatepriors', 0);
% toc;
% tic;
...
[R2,t2,X2,S2,a2,pk2,T2] = jrmpc_vis(V,X,'maxNumIter',refineIterMax, 'updatepriors', 0, 'r', R, 't', t, 's', S, 'initialpriors', pk');
[R3,t3,X3,S3,a3,pk3,T3] = jrmpc_vis(V,X,'maxNumIter',refineIterMax, 'updatepriors', 0, 'r', R, 't', t, 's', S, 'initialpriors', pk', 'vis', A_binary_c);
...
vis = genVisFromPeriod(st, length(vis), K);

[R4,t4,X4,S4,a4,pk4,T4] = jrmpc_soft(V,X,'maxNumIter',refineIterMax, 'updatepriors', 0, 'r', R, 't', t, 's', S, 'initialpriors', pk', 'vis', A_binary_c);
...
params.strIdx = strIdx;
h = figure;
drawTransformation(V, T3, params);
...
for epsilon = 0.01%[0.05 0.01 0.005 0.001 0.0005 0.0001]
...
id=9;
h=plotGMMK( id, gmmk, assignments3, maxIdx2, fv, I2, 2);
