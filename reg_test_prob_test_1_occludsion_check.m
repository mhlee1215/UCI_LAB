...
%     fv{data_idx} = stlread(sprintf('%s/%s_%.2f.%s', crop_path, data_name, dist_set{data_idx}, data_ext));
    ...
    dataRoot = 'E:/lab/work/2015_4d_seg/codes/data/desk_pose1';
    data3DFilePath = sprintf('%s/MeshedReconstruction%d.ply', dataRoot, data_idx);
    poseFilePath = sprintf('%s.pose', data3DFilePath);
    ...
    poseSetMatUnique = unique(poseSetMat, 'rows');
...
%     
    ...
    %Subsampling
    [sData, sIndex, sColor] = uniformSubSample(fv{data_idx}.Vertices, 40, fv{data_idx}.FaceVertexCData);
