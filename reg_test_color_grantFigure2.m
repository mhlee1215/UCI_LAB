...
for id=1:d_size
    ...
    GMM_color = [sRMean2 sGMean2 sBMean2];
    GMM_sum_color = GMM_sum_color + GMM_color;
    GMM_sum_index = GMM_sum_index + (sum(GMM_color==0, 2) < 3);
end
...
clusterSize = 25;
[centers, assignments] = vl_kmeans(AX, clusterSize);
...
for rr=1:rmax
    for cc=1:cmax
        ...
        hold on;
        assinIdx = find(assignments == pIdx);
        A_binary_sub = A_binary(:, assinIdx(1));
                
        plot(A_binary_sub, 'k', 'lineWidth', 2);
...
%         
%         plot(mean(mean(norm_a2'))+ones(size(norm_a2, 1), 1).*norm_var.*1, 'k', 'lineWidth', 2);
%         plot(mean(mean(norm_a2'))-ones(size(norm_a2, 1), 1).*norm_var.*1, 'k', 'lineWidth', 2);
%         
% %         vnorm_a2 = var(norm_a2');
% %         plot((vnorm_a2 - mean(vnorm_a2)).*max(max(norm_a2)) ./ sqrt(var(vnorm_a2)), 'B', 'lineWidth', 2);
%         
...
%         
%         m33 = mean(norm_a2, 2);
%         n33 = (m33 - mean(m33)) ./ sqrt(var(m33));
% %         plot(mean(mean(norm_a2'))+ones(size(norm_a2, 1), 1).*norm_var.*1, 'k', 'lineWidth', 2);
% %         plot(mean(mean(norm_a2'))-ones(size(norm_a2, 1), 1).*norm_var.*1, 'k', 'lineWidth', 2);
%         
% %         vnorm_a2 = var(norm_a2');
%         plot(n33, 'B', 'lineWidth', 2);
%         
%         title(sprintf('%d-th, size : %d, norm-var:%.2f, norm_dist', pIdx, sum(assignments == pIdx), norm_var));
%         hold off;
%         pIdx = pIdx+1;
%     end
% end
...
h=plotGMMK( id, gmmk, assignments, maxIdx, fv, I2);
title(sprintf('%d-th set, gmm group=%d', id, gmmk));
saveas(h, sprintf('%s/marked_set_%d_gmmk_%d%s_color', resultPath, id, gmmk, labels));
close(h);
h=plotGMMK( id, gmmk, assignments, maxIdx, fv, I2, 2 );
title(sprintf('%d-th set, gmm group=%d', id, gmmk));
saveas(h, sprintf('%s/marked_set_%d_gmmk_%d%s_gray', resultPath, id, gmmk, labels));
close(h);
...
h=plotGMMK( id, gmmk, assignments, maxIdx, fv, I2);
title(sprintf('%d-th set, gmm group=%d', id, gmmk));
saveas(h, sprintf('%s/marked_set_%d_gmmk_%d%s_color.png', resultPath, id, gmmk, labels));
close(h);
h=plotGMMK( id, gmmk, assignments, maxIdx, fv, I2, 2 );
title(sprintf('%d-th set, gmm group=%d', id, gmmk));
saveas(h, sprintf('%s/marked_set_%d_gmmk_%d%s_gray.png', resultPath, id, gmmk, labels));
close(h);


id=1;
gmmk=[2 3 4 7 10 11 12 15 16 17 18 19 24];
h=plotGMMK( id, gmmk, assignments, maxIdx, fv, I2);
title(sprintf('%d-th set, gmm group=%d', id, gmmk));
saveas(h, sprintf('%s/marked_set_%d_gmmk_%d%s_color.png', resultPath, id, gmmk, labels));
close(h);
