...
% [v, f, c] = loadData3D('bedroom_syn_parametric_1.klg.mesh.ply');
...
data_set = {'bedroom_syn_parametric_1.klg.mesh.ply', ...
            'bedroom_syn_parametric_2.klg.mesh.ply', ...
            'bedroom_syn_parametric_3.klg.mesh.ply', ...
            'bedroom_syn_parametric_4.klg.mesh.ply'};
d_size = length(data_set);
dataRoot = '~/lab_codes';
filePathSet={};
for i=1:length(data_set)
    filePathSet{end+1} = sprintf('%s/%s', dataRoot, data_set{i});
end
 
resultPath = 'results/EF_syn_test1';
mkdir(resultPath);
...
%     fv{data_idx} = stlread(sprintf('%s/%s_%.2f.%s', crop_path, data_name, dist_set{data_idx}, data_ext));
    ...
    data3DFilePath = filePathSet{data_idx};
    [v, f, c] = loadData3D(data3DFilePath);
%     [tri, pts, data, comments] = ply_read(data3DFilePath, 'tri');
%     
%     fprintf('end\n');
%     
